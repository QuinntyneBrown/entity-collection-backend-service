...
export class AppStore {
    ...
    public entityCollectionById$(id: string): Observable<EntityCollection> {
        return this._store.select("entityCollections")
            .map((data: { entityCollections: Array<EntityCollection> }) => {
                return pluck({ value: id, items: data.entityCollections }) as EntityCollection;
            })
    }

    public entityCollections$(): Observable<Array<EntityCollection>> {
        return this._store.select("entityCollections")
            .map((data: { entityCollections: Array<EntityCollection> }) => {
